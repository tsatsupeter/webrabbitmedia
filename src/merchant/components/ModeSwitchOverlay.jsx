...
export default function ModeSwitchOverlay() {
  ...
  const color = live ? 'emerald' : 'orange'
  return (
    <div
    ...
    >
      <div className="relative flex flex-col items-center gap-5 rounded-2xl border border-white/10 bg-merchant-panel/95 px-10 py-8 shadow-2xl animate-scale-in">
        <div className="relative w-16 h-16 flex items-center justify-center">
          <span
            className={`absolute inset-0 rounded-full bg-${color}-500/20 animate-ping`}
          />
          <span
            className={`absolute inset-2 rounded-full bg-${color}-500/30`}
          />
          <span
            className={`relative w-4 h-4 rounded-full bg-${color}-500 shadow-[0_0_20px_rgba(16,185,129,0.6)]`}
          />
        </div>

...
import { studioNavGroups } from './nav'
        ...
        <div className="absolute left-full top-0 ml-2 z-50 min-w-[180px] rounded-lg border border-merchant-border bg-merchant-panel shadow-2xl p-1.5">
          ...
          {item.children.map((child) => (
            <NavLink
              ...
              className="block px-2.5 py-2 rounded-md text-[0.82rem] no-underline text-white/70 hover:text-white hover:bg-white/[0.03]"
            >
              {child.label}
            </NavLink>
          ))}
...
export default function StudioSidebar({ onNavigate, compact = false, groups = studioNavGroups }) {
  return (
    <aside
    ...
    >
      <BusinessSwitcher compact={compact} next="/studio" />
      ...
      <div className="p-3 border-t border-merchant-border">
        <NavLink
          ...
          } rounded-xl bg-accent/10 border border-accent/25 hover:bg-accent/15 transition-colors`}
          title="Start a project"
        >
          <Icon name="sparkles" size={16} className="text-accent-bright shrink-0" />
          {compact ? (
            <span className="text-[0.65rem] text-white/60">New</span>
          ) : (
            <div className="min-w-0">
              <div className="text-[0.65rem] uppercase tracking-wide text-white/40">Web Rabbit Studio</div>
              <div className="text-[0.85rem] font-medium text-white truncate">Start a project</div>
            </div>
          )}
        </NavLink>
      </div>

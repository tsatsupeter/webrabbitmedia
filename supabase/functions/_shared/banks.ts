...
// `institution_code` is the 360Pay routing code used by name-verify,
// collections and disbursements.
export type Bank = { code: string; name: string; institution_code: string }
...
export function isValidBankCode(c: string) {
  return CODES.has(String(c || '').toUpperCase())
}
export function bankName(c: string) {
  return BANKS.find((b) => b.code === String(c || '').toUpperCase())?.name ?? null
}
// 360Pay routing code for one of our short bank codes.
export function bankInstitutionCode(c: string) {
  return BANKS.find((b) => b.code === String(c || '').toUpperCase())?.institution_code ?? null
}
